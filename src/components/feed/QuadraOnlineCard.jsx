...
export default function QuadraOnlineCard({ profile }) {
  ...
  const shown = [...onlineNow, ...activeToday].slice(0, 5)

  return (
    <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '1rem 1.1rem', marginTop: '1rem' }}>
      <p className="eyebrow" style={{ marginBottom: '0.75rem', color: colour }}>{quadra} quadra</p>
      ...
      {shown.length === 0 ? (
        ...
        <>
          <p style={{ fontSize: '0.8rem', color: 'var(--muted)', marginBottom: '0.65rem' }}>
            {onlineNow.length > 0
      ...
      <button
        ...
        style={{ display: 'inline-block', marginTop: '0.85rem', fontSize: '0.76rem', color: 'var(--accent)', textDecoration: 'underline', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
      >
        Chat in your quadra room →
      </button>
    </div>
  )
}
