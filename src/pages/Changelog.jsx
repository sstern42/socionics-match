...
export const ENTRIES = [
     {
       ...
       label: 'New',
       items: [
         'Ask the AI — after each answer, the Socionics assistant now suggests up to 3 natural follow-up questions as clickable chips, so you can keep the conversation going with one tap',
     ...
     },
     {
       ...
       label: 'New',
       items: [
         'Settings — a new "Sign-in" section shows which method you use to log in — email, Google, or Discord (or more than one, if they\'re linked to the same account)',
         'Sign in — you can now sign in or create your account with Discord, alongside Google. Signing in by email still works too — it\'s now tucked under a "Sign in with email instead" link so the one-tap options come first',
         'Sign in — the Google button now matches dark mode instead of showing as a bright white box',
     ...
     },
     {
       ...
       items: [
         ...
         'Settings — fixed account deletion failing for anyone who\'d earned any points',
       ],
     },
     {
       date: '2 July 2026',
     ...
     },
     {
       ...
       items: [
         ...
         'Get Typed — added new testimonials from Andrew and Sol, and turnaround times now read in working days so they don\'t look like calendar/wall-clock time',
         'Feed — the "Show anonymous" filter was confusingly named for what it did, so it\'s now called "Non-anonymous"',
         'Support & Feed — removed the old type-mug shop block; the feed slot now points to the boards instead',
         'Rooms — member names in the desktop Activity sidebar are now links to their profile',
         'Updates — links in founder posts are now clickable instead of plain text',
         'Messages — bubbles are wider on mobile and the reply/react/edit/delete buttons now sit below the bubble instead of squeezed in beside it',
         'Messages — unmatching from inside a conversation now removes it from your list right away instead of needing a reload',
         'Messages — fixed the conversation view sometimes settling short of the newest message when an image was still loading in',
         'Feedback button — now follows dark mode instead of always showing light colors',
         'Dashboard — your daily Ask AI usage count now stays current when you come back from asking a question',
         ...
         'Feed — the sidebar\'s "Profile viewers" number now links straight to your Views tab',
         'Feed — "Preview your card" now opens in a focused overlay instead of expanding inline',
         'Feed — on wide screens, a new right-hand rail shows recent site updates, live member/connection stats, and a Premium upsell for free members, with the feed column now centered between both sidebars',
         'Feed — your mini profile card now shows a Free tier / Premium badge',
         'Site banner — no longer dismissable, and now follows light/dark mode instead of always showing dark colors',
         'Feed — removed the sidebar\'s own announcement box now that the sitewide banner covers it',
         'Feed — "Your strongest matches" is back at the top of the feed on every screen size',
         'Stats & Profile pages — header and footer now stay fixed in place while the content scrolls, matching the rest of the site',
       ],
     },
     {
       date: '21 June 2026',
