...
### Added
- **Discord #signups notification now shows the signup device**: The `discord-notify` edge function's `🔔 New sign-up` message now includes what kind of device the new member joined from (e.g. `📱 Mobile (iOS)`, `💻 Desktop (Windows)`). Since a database webhook on `auth.users` has no access to the request's user agent, the device is detected client-side at signup (`src/lib/device.js`) and passed through the magic-link OTP call's `options.data`, which lands in `auth.users.raw_user_meta_data` and is therefore present on the webhook `record`. The edge function maps the small stable key to a friendly label. Sign-in flows that don't set it (e.g. Google) simply omit the device line rather than showing "unknown".
