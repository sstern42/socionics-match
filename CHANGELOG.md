...
- **Auth — Google sign-in button now follows the site theme**: The Google Identity Services button was hardcoded to `theme: 'outline'` (Google's white button), so on the dark UI it stood out as a stark white box next to the dark Discord button. Its styling can't be freely overridden (Google brand rules), but GIS ships a dark theme, so `src/pages/Auth.jsx` now picks `filled_black` when `html.dark` is set and `outline` otherwise, and re-renders via a `MutationObserver` on the root element's class if the theme is toggled while the page is open. Also added `logo_alignment: 'center'` so the logo and label group like the Discord button beside it. Switching to `filled_black` revealed a second problem: GIS wraps its rendered button in an element that carries a light background, so the dark button sat inside a white box. Added a scoped rule (`.google-btn-slot > div/iframe { background: transparent !important }` in `src/index.css`) to strip that wrapper's background — the button element itself is nested a couple of levels deeper (a `[role="button"]`), so its own styling is untouched. This targets Google's injected markup, so it may need revisiting if they change it.
