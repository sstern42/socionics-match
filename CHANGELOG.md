...
### Fixed
- **Every member's email address was readable by anyone, with no account**: `get_member_emails()` and `get_incomplete_signups()` were created by hand in the SQL editor and never committed, so neither had ever been read in a pull request. Both are `SECURITY DEFINER`, both read `auth.users` directly, and neither contained any authorisation check — while `EXECUTE` sat at the Postgres default of `PUBLIC`, plus explicit grants to `anon`. The `anon` key ships inside the frontend bundle by design, so a single unauthenticated `POST /rest/v1/rpc/get_member_emails` returned the id, email, name, type and signup date of every member; `get_incomplete_signups()` did the same for anyone who had started signing up in the previous 7 days. Both are called from `src/pages/Admin.jsx`, so they were always meant to be founder-only — the client gated the page and nothing gated the function. This is the identical defect `get_admin_stats()` had, fixed on that same Admin page in `20260623120000_get_admin_stats_role_check.sql`; its two neighbours were missed because they are not in this repo, which is exactly the risk `supabase/PRODUCTION_ONLY_OBJECTS.md` was written to describe. `EXECUTE` was revoked from `PUBLIC` and `anon` on production immediately, and migration `20260803140000_harden_production_only_rpcs.sql` adds the same founder check `get_admin_stats()` uses, plus those revokes so a rebuilt database doesn't reinherit the default grant.
- **`has_swiped_right()` let anyone check whether one member had liked another**: Both user ids came from the caller and neither was validated against `auth.uid()` — the same pattern already fixed twice, in the referral RPCs (`20260623123000`) and `log_profile_view()` (`20260623124000`). Profile ids are not secret; they appear in `/profile/:userId` URLs. The direction matters for the fix: the only callsite (`src/components/feed/SwipeDeck.jsx`) asks "has *this other member* swiped right on *me*", so the **target** is what must come from the session and the swiper stays a parameter — deriving the swiper instead is the obvious-looking fix and breaks the feature. `p_target_id` is now ignored, the signature kept so no client code changes.
- **`toggle_message_reaction()` didn't check the caller was in the conversation**: The caller was correctly derived from `auth.uid()`, but nothing verified they were a participant in the match the message belonged to, so any authenticated user holding a message id could attach reactions inside a conversation they had no part in. Message ids aren't guessable, which made this the least urgent of the four; the participant check costs nothing. Also gained the `SET search_path` it was missing. Verified on a throwaway Postgres 16 cluster: an ordinary member and an anonymous caller are both refused by the two admin functions while a founder still gets rows; a third party probing someone else's swipe pair now gets `false` while the legitimate "did they swipe me" call still returns `true`; and a non-participant is refused a reaction while a participant can still toggle one.
- **Message reactions were readable by anyone, and their write policies could never match**: `message_reactions` had `reactions_select` as `USING (true)`, so with `anon`'s SELECT grant the table was world-readable without an account — every `message_id` / `user_id` / `emoji` row across all private conversations. Reaction metadata rather than message content, but there was no reason for it to be public. Its `reactions_insert` and `reactions_delete` policies compared `user_id = auth.uid()`, and that column holds a `public.users.id` (what `toggle_message_reaction()` writes) while `auth.uid()` returns the auth id, so the two could never match. That failed closed, which is why nothing was exploitable — direct writes were impossible, and reactions worked only because `toggle_message_reaction()` is `SECURITY DEFINER` and bypasses RLS. The sibling `founder_post_reactions` already used the correct pattern. `20260803150000` restricts SELECT to participants of the conversation and rewrites both write policies to the `users.id` lookup used everywhere else. Verified on a throwaway Postgres 16 cluster acting as `anon` and `authenticated`: before, an anonymous caller could read every reaction; after, anon and outsiders see none while a participant sees theirs, an outsider's insert is rejected, and a participant inserting under someone else's `user_id` is rejected.
- **"AI used today" always showed 0 for free members**: `ai_message_counts` had RLS enabled with no policies at all, so every read from the browser returned nothing — and both call sites (`src/pages/HomeDashboard.jsx`, `src/components/SocionicsChat.jsx`) coalesced the empty result to `0`. They were also querying by the wrong id: the row is keyed on `users.id`, written by the `chat-socionics` edge function from `userRow.id`, while both reads passed the auth id. Two defects stacked, so fixing the policy alone would have changed nothing. Adds a self-read policy and corrects both call sites; `SocionicsChat` also gains `userId` in its effect dependencies, since that prop arrives a tick after mount and the read would otherwise be skipped on its only run. Writes deliberately get no policy — the count is maintained solely by `increment_ai_message_count()`, which is `service_role` only, so a member can see their usage and still not edit it. **The daily cap was never at risk**: it is enforced server-side by that same row-locked RPC (`20260623126000`), so the broken read cost an accurate indicator, not free messages.
- **socion.app silently stopped deploying — every production build was cancelled**: `netlify.toml`'s `ignore` command compared against `$CACHED_COMMIT_REF`, and Netlify's build cache is shared across deploy contexts. A Deploy Preview builds the PR head and leaves `CACHED_COMMIT_REF` pointing at it; when the PR merges, the production build compares the merge commit against that same PR head. Nothing else has landed in between, so the trees are identical, `git diff --quiet` finds no changes and exits 0 — which is exactly how Netlify is told to ignore a build. **Production cancelled on every merge while Deploy Previews succeeded**, so PRs looked healthy and the live site quietly stopped updating; the deploy list for 3 August shows five consecutive `Production: main@… Canceled` rows against five `Deploy Preview … Completed`. The rule has been in place since 2 July (#901), so the site had been frozen for a month: the `profile_views` grant fix was applied to the database but the matching frontend never shipped, which is why "Profile viewers" kept reading 0 from stale code even after the database was correct. Fixed by comparing against the commit's own first parent — `git diff --quiet "$COMMIT_REF^" "$COMMIT_REF" -- …` — which is context-independent and can't be poisoned by whatever built last: on a merge commit it's the previous `main`, on a branch push it's the previously built head, and a parentless commit makes `git diff` error non-zero, which builds. Verified by replaying the real rule under `sh` against every merge of 3 August: #983, #984, #985 and #987 build (all touch `src/`), and #986 still skips (`supabase/` and markdown only), preserving the build-minute saving the rule exists for.
- **"Profile viewers" always showed 0, and the premium "who viewed you" list was always empty**: `profile_views` was created by raw SQL in `20260606_profile_views.sql` with RLS enabled and both policies in place, but never received the baseline table `GRANT`s the `authenticated` role needs — the same omission the boards and rooms tables hit in `20260619130000_board_grants_fix.sql` and `20260619150000_room_grants_fix.sql`. Postgres raises `permission denied for table profile_views` *before* RLS is evaluated, so every read of the table failed. It failed invisibly, which is why it went unnoticed for two months: `getProfileViewCount()` caught the error and `return 0`, so the sidebar card read "Profile viewers 0" for everyone; and `getProfileViews()` threw, leaving the premium Views tab rendering its "No views yet." empty state. Writes were never affected — they go through `log_profile_view()`, which is `SECURITY DEFINER` and runs as the table owner — so the view history has been accumulating correctly all along and simply couldn't be read back. Migration `20260803130000_profile_views_grants.sql` grants `SELECT, INSERT` on `public.profile_views` to `authenticated` (no `UPDATE`/`DELETE` — view events are append-only; `INSERT` so the existing "insert own views" policy is reachable rather than being a policy that can never apply). `getProfileViewCount()` now throws instead of returning 0, since a swallowed error was indistinguishable from a genuine zero — both callers already render `—` when the query fails, so a broken read looks broken.
- **The schema couldn't be rebuilt from `supabase/migrations/` (issue #972)**: Nothing in the migrations directory ever created `users`, `matches`, `messages`, `blocks`, `stats` or `push_subscriptions` — those came from the hand-run files in `supabase/` that the README documented as "run this in the SQL editor". So replaying against an empty database died on the first statement of the first migration (`ERROR: relation "users" does not exist`), which is why the Supabase Preview check failed on every PR touching `supabase/` regardless of what the PR contained. Three new migrations carry the base schema into the sequence: `20260501000000_initial_schema.sql` (tables from `schema.sql`, `blocks.sql`, `push_subscriptions.sql`, `stats.sql`, plus `users.avatar_url`), and `20260527130000_baseline_rls_policies.sql` / `20260527140000_baseline_swipe_mode.sql` for `rls_reset.sql` and `swipes_schema.sql` — those two have to run *after* `20260527120000` because both call `can_add_connection()`, which that migration creates. That dependency is itself the tell that the hand-run files were edited after they were first run. Verified by replaying all 74 migrations onto an empty Postgres 16 with a Supabase-shaped baseline (roles, `auth` schema, `auth.uid()`, realtime publication).
- **Migration filenames now sort in authored order (issue #972)**: 18 files used a short date (`20260602_quadra_rooms.sql`) or no prefix at all (`add_room_message_image_url.sql`), and because `_` sorts after digits they ran late — `20260618_boards.sql` ran *after* `20260618160000_board_post_views.sql`, so a rebuild failed with `relation "board_posts" does not exist`. Three further pairs shared a timestamp and were ordered only by their description. All 20 are renamed to full unique 14-digit timestamps, using the exact versions the repo had already committed to in `20260629123000_create_schema_migrations_table.sql`'s backfill — checked so that all 47 versions recorded in production's `schema_migrations` still match a filename exactly, meaning production keeps seeing them as applied and a `supabase db push` won't re-run them.
- **Six objects that existed only in production are now reconstructed**: Beyond the missing base schema, migrations referenced objects nothing in the repo creates — `matches.unmatched_at`, `users.last_active`, `users.verified_by`, `stats.announcement`, `stats.announcement_active` and `get_my_user_id()`. Each is reconstructed in Section 5 of the initial migration from its usage, with the evidence recorded inline (e.g. `verified_by` is `text` rather than a uuid FK because `ProfileCard.jsx` renders it as `Verified by ${verified_by}`). Production's copies stay authoritative and untouched: the initial migration is never run there, only recorded as applied.
- **"Profile viewers" counted visits, not people**: The same count selected one row per view event, while both places it appears call it *viewers* ("Profile viewers" in the sidebar, "N people viewed your profile in the last 7 days" on the Views tab). `log_profile_view()` only deduplicates within a 1-hour window, so a single member checking your profile once a day would have been reported as 7 people. It now counts distinct `viewer_id`s over the window, matching the premium list, which was already deduplicated per viewer.
- **Points could be minted by anyone, for anyone (issue #970)**: `award_points(p_user_id, p_action_type, p_ref_id)` was `SECURITY DEFINER`, exposed as a PostgREST RPC, and executable by `PUBLIC` (the Postgres default) — so any signed-in user could `POST /rest/v1/rpc/award_points` and write straight to the ledger. Two holes: the target came from the caller-supplied `p_user_id`, never from the session, so points could be credited to *any* account; and the daily cap only covers the repeatable actions, leaving the high-value one-time ones (`mutual_match` 20, `referral_qualified` 30, `profile_complete` 50) guarded only by `UNIQUE (user_id, action_type, ref_id)` — which a fresh `p_ref_id` sidesteps on every call. Fifty calls in a loop was 1,000 points and the top of the leaderboard. Migration `20260803120000_award_points_derive_caller.sql` applies the same hardening the referral RPCs got in `20260623123000`: the original body moves to `award_points_internal()`, `REVOKE`d from `anon`/`authenticated`/`PUBLIC` so it's reachable only from other definer functions; the client-facing `award_points(action_type, ref_id)` takes no user id at all and derives the caller from `auth.uid()`; and the three-argument version is dropped outright rather than left in place accepting an ignored id. Mutual matches — the one case where a caller legitimately credits someone else — go through a new `award_match_points(match_id)`, which reads both participants off the match row after checking the caller is one of them. Client callsites in `src/lib/` no longer pass a user id.
- **Referral reward RPC had silently lost its caller check**: While fixing the above, found that `20260703140000_add_points_system.sql` re-created `grant_referral_reward()` from its *pre-hardening* definition in order to add the referrer's points award — quietly reverting the `auth.uid()` derivation added in `20260623123000` and reopening the referee-slot hijack documented there (an attacker who knows a victim's user id consumes the victim's one-time referee slot and credits themselves). The same migration restores the hardened body, with the points award retained.

### Added
- **`supabase/PRODUCTION_ONLY_OBJECTS.md`**: Verifying the rebuild turned up a wider gap than the base schema — 11 RPCs the app calls (`unmatch`, `mark_messages_read`, `toggle_save_profile`, `get_network_data`, …) and at least 7 tables (`ai_message_counts`, `feedback`, `founder_posts`, `notifications`, …) exist in production but in no file in this repo. `ai_message_counts` is the sharpest example: migration `20260623126000` inserts into and updates a table nothing ever creates. These are deliberately **not** reconstructed — most are `SECURITY DEFINER` functions where a subtly wrong body is a security bug rather than a typo, so the definitions have to come from production. The new doc inventories them and gives the exact SQL to dump the real definitions and commit them. It also records why a green replay doesn't prove they exist: Postgres doesn't validate a `plpgsql` body at creation time, so a migration defining a function against a missing table still applies cleanly. The gap was found by cross-checking every `.rpc()` and `.from()` call in `src/` and `supabase/functions/` against a rebuilt database.
- **Migration conventions in `CONTRIBUTING.md`**: unique 14-digit timestamps, idempotent statements, create everything in a migration rather than the SQL editor, and a reminder that a `supabase/` PR runs a billed preview database for as long as it stays open.
- **CI: lint and build now run on every PR (issue #971)**: `.github/` held only `FUNDING.yml`, so `npm run lint` and `npm run build` were never run automatically — Netlify builds only after merge, and its `ignore` rule skips builds when just `supabase`/`.github`/`.claude`/`*.md` changed, so a break landing alongside such a change could slip through. Added `.github/workflows/ci.yml` running both on `pull_request` and pushes to `main` (Node 20, `npm ci`, npm cache, plus a `concurrency` group so a new push cancels the in-flight run). The build step passes placeholder `VITE_SUPABASE_*` values — Vite inlines `import.meta.env` at build time and treats a missing `VITE_*` var as an empty string rather than an error, so no real credentials are needed to catch a broken build. Lint is left at its current settings (0 errors, 15 warnings) so it passes today and can be made a required check without a cleanup pass first; `--max-warnings 0` needs the remaining `react-hooks` warnings triaged first.

### Changed
- **The premium "who viewed you" list is now bounded**: `getProfileViews()` fetched every row a profile had ever accumulated — each joined to a `users` row for the viewer's name, type and avatar — and then collapsed them client-side to one entry per person. Harmless at today's volume (the whole table holds ~218 events, and the busiest profile has 40), but it grows without limit and the work is wasted: most of what's fetched is discarded by the dedup. Now capped at `ROW_BUDGET` 500 view events fetched and `VIEWER_LIMIT` 50 distinct viewers returned. Two limits rather than one because dedup happens after the fetch, so repeat visits are spent out of the row budget — `log_profile_view()` dedupes only within a 1-hour window, and at ~2.1 events per viewer site-wide a 10x budget leaves generous headroom. A profile whose viewers are unusually loyal returns fewer than 50 names rather than wrong ones. Not user-visible: no profile is near either bound. The count in `getProfileViewCount()` is deliberately left unbounded — capping it would make the number wrong rather than cheaper, and it selects a single column with no join.
- **"Profile viewers" is now an all-time count, not a 7-day one**: With the grant fixed and the count reading real data, the number turned out to disagree with the page it links to. `getProfileViewCount()` looked back 7 days, but `getProfileViews()` — the premium "who viewed you" list the sidebar card navigates to via `?tab=views` — never filtered by date at all. On the founder account that meant the card read "Profile viewers 0" while the list one click away held 19 people. Free members saw the mismatch in the other direction, since their Views tab shows the windowed count with matching copy. No setting of the window fixes this while the list is unwindowed, so the window is gone: `getProfileViewCount()` now counts distinct `viewer_id`s over all time, matching the list. The volume argues the same way — the whole `profile_views` table holds ~218 events across nine weeks, so a 7-day window reads 0 on a normal week for most members: accurate, and useless. The free-tier Views tab copy drops "in the last 7 days" accordingly.
- **One canonical `.env.example`, and the Giphy key is documented (issue #973)**: `src/components/GifPicker.jsx` reads `VITE_GIPHY_API_KEY`, which appeared in no example file and no docs — a fresh clone got a GIF picker that 401s and says "Could not load GIFs." with no hint why. Worse, the repo carried *two* divergent example files (`.env.example` with 2 vars, `env.example` with 3), so a contributor couldn't tell which to copy and neither was complete. Consolidated into a single commented `.env.example` listing all five vars the app actually reads — including `VITE_GOOGLE_CLIENT_ID`, which the README documented but neither example file had — with `env.example` deleted, and a required/optional table added to the README (the deploy section now points at it rather than repeating a shorter list) and to `CONTRIBUTING.md`. The GIF affordance now degrades gracefully too: a shared `src/lib/giphy.js` exports `GIF_ENABLED`, and the GIF button is hidden in both messages and rooms when the key is absent, instead of offering a picker that can only fail.
...
### Fixed
- **Admin dashboard — "Unblock" now actually lifts the block**: Clicking Unblock on a report in the admin dashboard appeared to succeed but the block stayed in place after reload. Founders can *see* every block via the `get_admin_stats()` RPC, but the "Unblock" action performed a direct client update (`liftBlock`) that is still subject to row-level security. A founder is neither the blocker nor the blocked party, so the update matched zero rows — no error, no change. This is now done through a new `admin_lift_block()` `SECURITY DEFINER` RPC that performs its own founder check and bypasses RLS entirely (the same pattern `get_admin_stats()` uses), so the lift reliably persists. (A founder UPDATE policy was also added on `blocks` in the same change, but the RPC is what the admin action now uses.)
