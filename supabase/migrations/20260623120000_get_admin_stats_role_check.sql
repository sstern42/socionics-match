-- Security fix: get_admin_stats() had no server-side role check. It is
-- SECURITY DEFINER with no REVOKE EXECUTE, so any authenticated user could
-- call supabase.rpc('get_admin_stats') directly and receive the full admin
-- dashboard payload (unresolved abuse reports with names, referral
-- financial data, aggregate user counts). The only prior gate was the
-- client-side route check in src/pages/Admin.jsx, which a direct RPC call
-- bypasses entirely.
--
-- This adds the same role check used by the client gate, enforced inside
-- the function body so it can't be bypassed.
...
begin
  ...
  select jsonb_build_object(
    ...
    'user_reports',      (
                           select coalesce(jsonb_agg(r order by r.created_at desc), '[]')
                           from (
                             select ur.id, ur.reporter_id, ur.reported_user_id, ur.reason, ur.notes,
                                    ur.created_at, ur.resolved_at, ur.resolution,
                                    reporter.profile_data->>'name' as reporter_name,
                                    reported.profile_data->>'name' as reported_name
                             from user_reports ur
                             left join users reporter on reporter.id = ur.reporter_id
                             left join users reported on reported.id = ur.reported_user_id
                             where ur.resolved_at is null
