-- ============================================================================
-- Migration: baseline swipe mode (swipes_schema.sql) — issue #972
-- ============================================================================
-- Content is swipes_schema.sql verbatim, moved into the migration sequence so
-- a from-scratch rebuild reproduces it: the swipes table, its RLS policies,
-- the matches_pair_unique index, the mutual-match trigger and the
-- already_swiped view.
--
-- Like the RLS baseline before it, this has to run after 20260527120000:
-- handle_mutual_swipe_match() calls can_add_connection() to respect the
-- free-tier connection cap on both sides of a mutual swipe.
--
...
-- Safe to re-run regardless: creates are IF NOT EXISTS / OR REPLACE and the
-- policies and trigger are dropped-if-exists first.
-- ============================================================================
